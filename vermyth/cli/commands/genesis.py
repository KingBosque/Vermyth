...
def cmd_review_genesis(
    cli: "VermythCLI", *, genesis_id: str, reviewer: str, note: str | None = None
) -> None:
    try:
        out = cli._tools.tool_review_genesis(
...
def register_subparsers(subs: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    pg.add_argument("--min-coherence-rate", type=float, default=0.6, dest="min_coherence_rate")
    ...
    rv = subs.add_parser("review-genesis", help="Review an emergent aspect proposal.")
    rv.add_argument("genesis_id", metavar="GENESIS_ID")
...
def _dispatch_propose_genesis(cli: "VermythCLI", ns: argparse.Namespace) -> None:
    cli.cmd_propose_genesis(
        ...
        min_coherence_rate=float(ns.min_coherence_rate),
    )
...
def _dispatch_review_genesis(cli: "VermythCLI", ns: argparse.Namespace) -> None:
    cli.cmd_review_genesis(genesis_id=ns.genesis_id, reviewer=ns.reviewer, note=ns.note)
...
DISPATCH = {
    ...
    "review-genesis": _dispatch_review_genesis,
