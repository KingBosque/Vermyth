...
- `GET /arcane/telemetry`: read-only **bundle adoption telemetry** summary (in-memory; requires `VERMYTH_BUNDLE_TELEMETRY=1` to have recorded events). Same aggregate as MCP `get_bundle_adoption_telemetry`. No network export.
- `GET /arcane/telemetry/report`: **derived adoption report** (funnels per bundle, tops, bytes estimates, heuristic findings). Same content as MCP `get_bundle_adoption_report` and `vermyth arcane-bundle-report --json`. Ratios are approximate (event counts, not unique sessions).
