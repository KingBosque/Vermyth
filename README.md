...
- **Recommend bundles (advisory):** MCP **`recommend_semantic_bundles`** or HTTP **`POST /arcane/recommend`** with `{ "skill_id", "input" }` returns **inspectable** matches (`strength`, `match_kind`, `matched_features`, `why_better`, **`guided_upgrade`**) for plain JSON—**no execution**, never rewrites your request. Each hit includes a **copy-paste `semantic_bundle` ref** (placeholder params), **inspect** hints (MCP tool args, HTTP path, `vermyth://` resource URI), and the **target skill**—so you can jump to **`inspect_semantic_bundle`** / **`GET /arcane/bundles/{id}`** and then invoke. Matching is **manifest-driven** (`recommendation` in bundle JSON). Use a higher `min_strength` (optional) to filter weak hints. Catalog rows include a compact **`recommendation`** hint when present; manifests may set **`"library": "canonical"`** for curated defaults.
- **Recommended bundle-first workflows** (plain JSON remains fully supported; see [`docs/http_adapter.md`](docs/http_adapter.md) and [`docs/specs/arcane-design-summary.md`](docs/specs/arcane-design-summary.md)):
  - **`coherent_probe`** (`decide`) — MIND+LIGHT policy probe; shorter than hand-written `intent` + `aspects`.
  - **`strict_ward_probe`** (`decide`) — ward thresholds (0.92 / effect-risk) without merging `PolicyThresholds` by hand.
  - **`divination_gate`** (`decide`) — divination / review gate; enforces causal context via expansion.
  - **`network_edge_ward`** (`decide`) — VOID+MIND boundary decisions with tight effect-risk gates for external-facing work.
  - **`resonance_ping_cast`** (`cast`) — repeatable MIND+LIGHT health / resonance ping with a stable objective prefix.
  - **`compile_single_cast_program`** (`compile_program`) — minimal one-node semantic program preset for `compile_program` (no advisory recommendation; discover via catalog).
- **Bundle adoption telemetry (local, opt-in):** set **`VERMYTH_BUNDLE_TELEMETRY=1`** to record in-memory counters for recommendation, catalog/inspect, invocation, and (with **`VERMYTH_BUNDLE_TELEMETRY_MISSED=1`**) “missed upgrade” cases where plain JSON matched a bundle but no `semantic_bundle` was used. No network export. Inspect via MCP **`get_bundle_adoption_telemetry`** or HTTP **`GET /arcane/telemetry`**. **Derived report:** MCP **`get_bundle_adoption_report`**, HTTP **`GET /arcane/telemetry/report`**, CLI **`vermyth arcane-bundle-report`**, or Python **`vermyth.arcane.build_bundle_adoption_report()`** — funnels, tops, bytes-by-bundle estimates, and heuristic **findings** (not authoritative). See [`docs/specs/arcane-design-summary.md`](docs/specs/arcane-design-summary.md#bundle-adoption-telemetry-local).
- MCP tools: **`list_semantic_bundles`**, **`inspect_semantic_bundle`**, **`recommend_semantic_bundles`**, **`get_bundle_adoption_telemetry`**, **`get_bundle_adoption_report`**, **`expand_semantic_bundle`**, **`compile_ritual`**. Operational spec: [`docs/specs/ontology.md`](docs/specs/ontology.md); design digest: [`docs/specs/arcane-design-summary.md`](docs/specs/arcane-design-summary.md).
